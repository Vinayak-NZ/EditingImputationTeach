...
Editing and imputing are both methods of data processing. Editing refers to the detection and correction of errors in the data. Whilst imputation is a method of correcting errors in a dataset. This document addresses the following questions:  
...
kable(DataK_editSub[1:5,], caption="First five units of the Income dataset")
...
1) Review: data is examined for potential problems; identifying instances of missingness and incorrect responses  
2) Select: data is identified for further treatment. Of the potential problems identified in the review phase, a method is applied to determine which of these erroneous cases need to corrected or which missing values need to be estimated  
...
#### How to review data?
Data can be reviewed using a variety of methods. Table 1 presents a list of data review methods, along with definitions and the utility of each in identifying problems in the dataset. The types of review methods include:  
...
### Select
#### What to consider?
In the select phase, units or fields within units are identified for further treatment; to either change incorrect values or fill in missing values. When selecting data items, it is important to consider the volume of items being identified for treatment, and the potential impact of each unit on the quality of the data.  
  
![](Select.png)  
  
A key objective when selecting data is to change the fewest possible data items to manage an error source. For example, if a unit has provided responses to items A , B and C; whereby items A and B should equal C, but in this case do not, the following set of data items could be selected for amending:  
1) A  
2) B  
3) C  
4) A and B  
...
kable(SelDat, caption="An error in a reocrd")
...
#### How to select data?
Methods for selecting data can involve either:   
...
Table 2 details the methods of unit and variable selection.  
  
![](SelectMethods.png)
  
### Amend
#### What to consider?
In the amend phase, the objective is to change the data values in order to improve data quality. Units/ variables that were flagged in the select phase are amended to produce a transformed dataset, which should better meet the needs of customers.  
...
4) Knowing what data is available to derive estimates of missing or incorrect units/ values. Data that could be used in the amend phase includes the units and variables in the dataset of interest, as well as other related data sources that analysts might have access to.  
...
7) Ensuring that all edit rules applied are still relevant and accurate for the statistical production. Edit rules that are built into the design of a production cycle should be reviewed; to determine whether the type or mechanism of the error it is addressing has not changed, and whether the edit rules needs to be adapted.  
  
#### How to amend data?
The aim of amend phase is to improve the quality of data by addressing the missing and erroneous values and units. By managing the impact of the error sources, the ability to draw inferences of the target population and target concept is in turn improved. The amend functions can be categorised as follows:  
...
Table 3 presents the methods for variable and unit amendment.
